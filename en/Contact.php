...
if(isset($_POST['submitted'])) {
	...
	// captcha 
	if (empty($_POST['code'])) {
		$captchaError = 'You have to enter the Captcha Code.';
		$hasError = true;
	...
	// need valid email
	if(trim($_POST['email']) === '')  {
		$emailError = 'Please enter you Mailadress.';
		$hasError = true;
	} else if (!preg_match("/^[[:alnum:]][a-z0-9_.-]*@[a-z0-9.-]+\.[a-z]{2,4}$/i", trim($_POST['email']))) {
		$emailError = 'Please enter a valid Mailadress.';
		$hasError = true;
	} else {
	...
	if(trim($_POST['comments']) === '') {
		$commentError = 'Please write a short Message!';
		$hasError = true;
	} else {
...
<article>
  ...
  <div class="left cont_left shadowed">
    <table>
	  ...
	  <meta http-equiv="refresh" content="3; url=<?php $_SERVER['SCRIPT_NAME'] ?>" />	  
    <div class="right cont_right shadowed" style="widht:240px;margin:180px 0 0 20px;">
      <div id="contact" class="section">
        <div class="container content">
          ...
          <p style="margin:50px 0;" class="left boldfirst">Thanks, <br /><br />You Message was <br />succsesfull sent.</p>
        </div>
      </div>
    </div>
...
<!--          <p class="alert">Error submitting the form</p> -->
		...
		<?php } ?>
	      ...
	      <div class="formblock">
		<label class="screen-reader-text"></label>
		<input type="text" name="email" id="email" value="<?php if(isset($_POST['email']))  echo $_POST['email'];?>" class="txt requiredField email <?php if($emailError != '') { echo 'inputError'; } ?>" placeholder="Mail:" />
		<?php if($emailError != '') { ?>
		...
		<br /><a href="javascript:P91Captcha('<?=$sessionstringadd;?>');">New Code?</a>
		<br />
		<?php if($captchaError != '') { ?>
		...
		$('form#contact-us').submit(function() {
			...
			$('.requiredField').each(function() {
				if($.trim($(this).val()) == '') {
					...
					$(this).parent().append('<span class="error">You have to fill '+labelText+'.</span>');
					...
					if(!emailReg.test($.trim($(this).val()))) {
						var labelText = $(this).prev('label').text();
						$(this).parent().append('<span class="error">Sorry! The entered Mailadress '+labelText+' is not valid.</span>');
						$(this).addClass('inputError');
						hasError = true;
					}
				} else {
					...
					if (sha1(trim(strip_tags(strtoupper($_POST['code'])))) != $_SESSION['P91Captcha_code']) {
						var labelText = $(this).prev('label').text();
						$(this).parent().append('<span class="error">Sorry! The used '+labelText+' isnot valid.</span>');
						$(this).addClass('inputError');
						hasError = true;
					}
				}
			...
			if(!hasError) {
				...
				$.post($(this).attr('action'),formInput, function(data){
					$('form#contact-us').slideUp("fast", function() {				   
						$(this).before('<p class="tick"><strong>Thanks!</strong> You Message has been send succsessfully.</p>');
					captcha_});
				});
			}
